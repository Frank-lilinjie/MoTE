...
class Block(nn.Module):
    ...
    def forward(self, x, adapters=None):
        ...
        Args:
            ...
            adapters (list of Callable or None): 多个 adapter，每个 adapter 接收 x 并输出一个特征。
        ...
        # 如果提供了多个 adapter，则融合它们的输出
        if adapters is not None and len(adapters) > 0:
            adapter_outputs = []
            for adapter in adapters:
                adapter_output = adapter(x, add_residual=False)
                adapter_outputs.append(adapter_output)
            
            # 将多个 adapter 特征堆叠后，融合为单个特征
            adapter_outputs = torch.stack(adapter_outputs, dim=0)  # (num_adapters, batch_size, seq_len, dim)
            adapter_fused = adapter_outputs.mean(dim=0)  # 简单平均融合

            # 将融合后的 adapter 特征加回到主模型特征上
            x = x + adapter_fused
...
class VisionTransformer(nn.Module):
    ...
    def add_adapter_to_list(self, add_adapter = True):
        if add_adapter:
            ...
            self.add_adapter = False
        logging.info(f"Number of adapter: {len(self.adapter_list)}")
    ...
    def forward_train(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            adapter_list_blk = [task_adapter[idx] for task_adapter in self.adapter_list]
            if self.add_adapter:
                adapter_list_blk.append(self.cur_adapter[idx])
            if self.config.vpt_on:
                eee = self.embeddings[idx].expand(B, -1, -1)
                x = torch.cat([eee, x], dim=1)

            x = blk(x, adapter_list_blk) # 传入的是一个 block 中的adapter列表
            if self.config.vpt_on:
...
def vit_base_patch16_224_mofe(pretrained=False, **kwargs):
...
def vit_base_patch16_224_in21k_mofe(pretrained=False, **kwargs):
